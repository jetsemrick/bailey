...
DROP FUNCTION IF EXISTS reorder_flow_tabs(jsonb);
...
CREATE OR REPLACE FUNCTION reorder_flow_tabs(
  updates jsonb
)
RETURNS void
LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION reorder_flow_tabs(jsonb) TO authenticated;

COMMENT ON FUNCTION reorder_flow_tabs(jsonb) IS 
